...
ACTION UserGeneratedTokens::issue(name to, asset quantity, string memo) {
...
void UserGeneratedTokens::add_balance(name owner, asset value, name ram_payer) {
...
ACTION UserGeneratedTokens::open(name owner, symbol_code symbol, name ram_payer) {
...
ACTION UserGeneratedTokens::close(name owner, symbol_code symbol) {
    require_auth(owner);
...
EOSIO_DISPATCH(eosio::UserGeneratedTokens, (create)(issue)(transfer)(transferbyid)(open)(close)(retire))
